...
function makeBlindIndex(text: string): string {
  if (!text) return '';
  return crypto
...
async function main() {
  const rootEmail = process.env.ROOT_OWNER_EMAIL || 'tanish.thakare2005@gmail.com';
  const rootPassword = process.env.ROOT_OWNER_PASSWORD || 'GaneshaMoto@1234';
  ...
  console.log('Root owner created successfully.');
}
