...
const fromEmail = process.env.SMTP_FROM || 'Green Orbit Energy <noreply@greenorbitenergy.com>';
...
export async function sendVerificationEmail(to: string, token: string) {
  const subject = 'Verify Your Email Address – Green Orbit Energy';
  ...
  const body = `Dear ${userName},\n\nWelcome to Green Orbit Energy! Thank you for creating an account with us.\n\nTo complete your registration, please verify your email address using the OTP below:\n\nVerification Code: ${token}\n\nThis code is valid for 10 minutes.\n\nIf you did not create an account with Green Orbit Energy, please ignore this email.\n\nBest regards,\n\nTeam Green Orbit Energy\nSolar Maintenance & Services\n🌐 https://greenorbitenergy.com`;
  ...
  const html = `
    <div style="font-family: sans-serif; padding: 25px; border: 1px solid #e2e8f0; max-width: 600px; border-radius: 12px; color: #1e293b; line-height: 1.6;">
      <div style="text-align: center; margin-bottom: 20px;">
        ...
        <p style="color: #64748b; font-size: 14px; margin-top: 5px;">Green Orbit Energy Solar Maintenance & Services</p>
      </div>
      <p style="font-size: 15px;">Dear <strong>${userName}</strong>,</p>
      <p style="font-size: 15px;">Welcome to Green Orbit Energy! Thank you for creating an account with us.</p>
      ...
      <p style="font-size: 14px; color: #64748b; margin-top: 25px; border-top: 1px solid #e2e8f0; padding-top: 15px;">If you did not create an account with Green Orbit Energy, please ignore this email.</p>
      <p style="font-size: 14px; color: #475569; margin-top: 20px; line-height: 1.5;">
        Best regards,<br>
        <strong>Team Green Orbit Energy</strong><br>
        Solar Maintenance & Services<br>
        🌐 <a href="https://greenorbitenergy.com" style="color: #0284c7; text-decoration: none;">https://greenorbitenergy.com</a>
      </p>
    </div>
  `;
...
export async function sendInvoiceEmail(to: string, invoiceNumber: string, amount: number, pdfUrl?: string) {
  const subject = `Your Green Orbit Energy Invoice: ${invoiceNumber}`;
...
export async function sendPasswordResetEmail(to: string, token: string) {
  const subject = 'Reset Your Green Orbit Energy Password';
  ...
  const html = `
    <div style="font-family: sans-serif; padding: 25px; border: 1px solid #e2e8f0; max-width: 600px; border-radius: 12px; color: #1e293b; line-height: 1.6;">
      ...
      <p style="font-size: 15px;">Dear <strong>${userName}</strong>,</p>
      <p style="font-size: 15px;">We received a request to reset your Green Orbit Energy account password.</p>
