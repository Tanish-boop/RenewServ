...
def process():
    src_path = r"C:\Users\hp\.gemini\antigravity\brain\65ad541f-1201-414e-a9ce-9a37d2a45228\media__1784718590226.jpg"
    ...
    # We will ignore rows >= 619 to cut off the bottom noise border
    max_height = 619

    # 1. Create logo.png with transparent background
    print("Generating logo.png...")
    logo = Image.new("RGBA", (width, max_height))
    logo_data = []
        ...
        for x in range(width):
            ...
            if r > 240 and g > 240 and b > 240:
                ...
                if y >= globe_cutoff:
                    ...
                    # Globe section -> keep original green/blue
                    logo_light_data.append((r, g, b, a))
                    
    logo_light = Image.new("RGBA", (width, max_height))
